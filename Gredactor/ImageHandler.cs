...
/// АТТЕНШ ПЛИЗ
/// Реализация выделения области на форме, честно говоря, не удалась. Найти проблему вовремя не удалось. 
/// В связи с этим, несмотря на наличие в данном классе методов для работы с областью интереса,
/// эта функциональная возможность отключена на уровне формы.
/// Для включения следует в файле дизайнера формы раскомментировать строки, подписывающие элемент
/// pictureBox на события MouseDown, MouseMove и MouseUp.
/// Просьба не глумиться, я правда не знаю, что не так.
/// Спойлер: первое выделение отрабатывает прекрасно. Если применить какой-либо фильтр к тому же выделению, либо к новому (неважно),
/// картинка фейлится. Чаще всего она преобразуется в серый по неизвестным причинам. 
/// В случае поворота и масштабирования появляются линии разрыва в середине.
/// Если снять выделение, то все работает нормально.

namespace Gredactor
{
    /// <summary>
    /// Хранит и обрабатывает изображение, также занимается выделением (зона интереса)
    /// </summary>
    public class ImageHandler
    {
        ...
        public void Save()
        {
            ...
            {
                // баг в .NET, запрещающий сохранять в то же место, откуда было открыто изображение
